...
class coco(datasets.imdb):
    ...
    def _filter_crowd_proposals(self, roidb):
        """
        ...
        for ix, entry in enumerate(roidb):
            ...
            crowd_boxes = _xyxy_to_xywh(entry['boxes'][crowd_inds, :])
            non_gt_boxes = _xyxy_to_xywh(entry['boxes'][non_gt_inds, :])
            ious = COCOmask.iou(non_gt_boxes, crowd_boxes, iscrowd)
            bad_inds = np.where(ious.max(axis=1) >
                                self.config['crowd_thresh'])[0]
    ...
    def _load_proposals(self, method, gt_roidb):
        ...
        for i, index in enumerate(self._image_index):
            ...
            box_file = osp.join(self._data_path, 'proposals',
                                method, 'mat',
